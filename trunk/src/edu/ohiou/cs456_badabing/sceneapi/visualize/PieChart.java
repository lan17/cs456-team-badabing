...
public class PieChart extends ANode
{
    ...
    */
    ...
    private static IntBuffer indecies;
    private FloatBuffer colors_arr;
    ...
    public synchronized void render( GLAutoDrawable panel )
    {
       // panel.setGL(new DebugGL(panel.getGL()));
        ...
        if( need_to_build_colors && VBOsupported )
        {
            colors_arr = null;
            colors_arr = BufferUtil.newFloatBuffer( vertexCount * 3 );
            int color_count = -1;
            boolean need_to_change_colors = true;
            Color cur_color = null;
            double share = 0;
            int old_i = 0;
            for( int i = 0; i < mesh.size(); ++i )
            {
                if( need_to_change_colors )
                {
                    color_count++;
                    cur_color = colors[color_count];
                    if (color_count >= slices.size())break;
                    share = d_size_mesh * slices.get( color_count );
                }
                if( i - old_i > share )
                {
                    need_to_change_colors = true;
                }
                for( int j = 0; j < 18*3; ++j )
                {
                    colors_arr.put( cur_color.getRed() / 255f );
                    colors_arr.put( cur_color.getGreen() / 255f );
                    colors_arr.put( cur_color.getBlue() / 255f );
                   // colors_arr.put( mtl.color[3] );
                }
            }
            colors_arr.rewind();
            need_to_build_colors = false;
        }
//        System.out.println( "Displaying pie chart" );
        ...
        if( show )
        {
            ...
            if( VBOsupported )
            {
                ...
                gl.glColor3f( 0, 1, 0 );
                //gl.glDrawArrays( GL.GL_TRIANGLES, 0, 4530);
                gl.glColor3f( 1, 0, 0 );
                //System.out.println( vertices.toString() );
                //gl.glDrawArrays( GL.GL_TRIANGLES, 4527, 7548 );
