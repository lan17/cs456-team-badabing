            ...
            while (input.hasNext())
            {
                ...
                if ( line.size() > 1 )
                {
                    ...
                    if (line.elementAt(0).equals("v"))
                    {
                        ...
                        if( x_min > tmp.getX() ) x_min = tmp.getX();
                        if( x_max < tmp.getX() ) x_max = tmp.getX();
                        if( y_min > tmp.getY() ) y_min = tmp.getY();
                        if( y_max < tmp.getY() ) y_max = tmp.getY();
                        if( z_min > tmp.getZ() ) z_min = tmp.getZ();
                        if( z_max < tmp.getZ() ) z_max = tmp.getZ();
                    }
